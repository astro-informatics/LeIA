...
def test_np_vs_TF(shape_x, shape_y, NUFFT2D_op, NUFFT2D_TF_op, NNFFT2D_op, NNFFT2D_TF_op):
    ...
    assert np.allclose(NUFFT2D_op.dir_op(x), NUFFT2D_TF_op.dir_op(x_TF).numpy()), "nufft forward not the same"
    assert np.allclose(NUFFT2D_op.adj_op(y), NUFFT2D_TF_op.adj_op(y_TF).numpy()), "nufft adjoint not the same"
    ...
    assert np.allclose(NNFFT2D_op.dir_op(x), NNFFT2D_TF_op.dir_op(x_TF).numpy()), "nufft forward not the same"
    assert np.allclose(NNFFT2D_op.adj_op(y), NNFFT2D_TF_op.adj_op(y_TF).numpy()), "nufft adjoint not the same"
