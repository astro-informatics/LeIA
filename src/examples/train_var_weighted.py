...
from src.networks.GUNet_var_weighted import GUNet_var_weighted
...
yzx_known = Known("NUFFT_Random").unbatch().batch(batch_size)
...
#     model.fit(yzx_fixed, steps_per_epoch=set_size//batch_size, epochs=epochs, callbacks=[csv_logger, cp_callback]) #, validation_data=yzx_validation, validation_steps=set_size//batch_size)
    ...
    if mode == 4:
        ...
        print("fitting to known test sampling distribution")
        model.fit(yzx_known, steps_per_epoch=set_size//batch_size, epochs=epochs, callbacks=[csv_logger, cp_callback])
...
w_train = np.zeros_like(y_dirty, dtype=np.float32)
w_train[:, :] = calc_w(uv_test)

w_test = np.zeros_like(y_dirty_test, dtype=np.float32)
...
print("predict train")
train_predict = model.predict([y_dirty, sel_dirty, w_train], batch_size=batch_size, callbacks=[])
print("predict test")
test_predict = model.predict([y_dirty_test, sel_dirty_test, w_test], batch_size=batch_size)
