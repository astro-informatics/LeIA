...
from src.data.SPIDER_datasets import SATSDataset
from src.dataset import center_crop, measurement_func, random_crop
...
ds = SATSDataset()
ds = ds.map(center_crop).map(tf_func_original).take(480)
...
folder = f"./data/intermediate/SATS/NUFFT_SPIDER/"
