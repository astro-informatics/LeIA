...
def RadialSampling(n_spokes=37, n_bins=10):
    """Returns the u,v coordinates for sampling on radial spokes. Normalised to [-pi, pi]"""
    r   = np.linspace(0, np.pi,   n_bins, endpoint=False)
...
def SpiderSampling(n_spokes=37, n_lenslets=24, spectral_bins=10, max_baseline=0.65, min_baseline=0.15):
    """Returns the u,v coordinates for sampling following the SPIDER instrument. Normalised to [-pi, pi]"""
    ...
    for n in range(n_spokes):
        for i in range(n_lenslets//2):
            ...
            for k in range(spectral_bins):
                vis.append(2*np.pi*(x1-x2)/(500e-9 + (900e-9-500e-9)*(k/(spectral_bins - 1)) )) 

    vis = np.array(vis)
    vis = vis / np.max(np.sqrt(vis[:,0]**2 + vis[:,1]**2)) * np.pi
    return vis
...
def solver(y, phi=None):
    ...
    options = {'tol': 10e-5, 'iter': 5000, 'update_iter': 50, 'record_iters': False}
    ISNR = 20.
    sigma = 10**(-ISNR / 20.)
    size = len(y)
