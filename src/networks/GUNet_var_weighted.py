...
class sub_Gradient(tf.keras.layers.Layer):
    ...
    def __call__(self, x, y, sel, measurement_weights=1):
        ...
        sub_weights = tf.boolean_mask(measurement_weights, tf.reshape(sel[0], [-1]), axis=1)
#         tf.print((res.shape, sub_weights.shape, tf.reshape(measurement_weights, [1,-1]).shape))
        
        grad = self.m_op.adj_op_sub(res*sub_weights , tf.reshape(sel[0], [-1]))
...
class GUNet_var_weighted(tf.keras.Model):
    def __init__(
        self, 
        image_shape, 
        uv, 
...
#             x = tf.math.real(ops[0].adj_op(inputs * freq_weights[0]))
            ...
            measurement_weights = tf.keras.Input([ops[0].n_measurements  ], dtype=tf.complex64)

            print(inputs.shape, measurement_weights.shape)
...
#             x = tf.math.real(m_op.adj_op_sub(inputs * measurement_weights), sel)
        ...
        for i in range(depth+1): 
            ...
            subsampled_weight = tf.boolean_mask(measurement_weights, low_freq_sels[i], axis=1)
            subsampled_weight.set_shape([batch_size, np.sum(low_freq_sels[i])])         
            subsampled_weights.append(subsampled_weight)           
        ...
        freq_weights = subsampled_weights
        ...
        super().__init__(inputs=[inputs, sel, measurement_weights], outputs=outputs)
