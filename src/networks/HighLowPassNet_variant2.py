...
class UpSample(tf.keras.layers.Layer):
    ...
    def __call__(self, low_freq_image, high_freq_measurements, measurement_weights=1):
        ...
        # high_freq_measurements = tf.tensor_scatter_nd_update(
        #     full_freq_info,
        #     self.indices,
        #     tf.zeros_like(measurements)
        # ) # update the lower coefficients with low-scale passed information
        
        low_freq_measurements = tf.zeros_like(high_freq_measurements)
        low_freq_measurements = tf.tensor_scatter_nd_update(
            low_freq_measurements,
            self.indices,
            measurements
        ) # 

        up_sampled_image = self.high_scale_op.adj_op( low_freq_measurements * measurement_weights )
    ...
    #     low_freq_measurements =  tf.tensor_scatter_nd(
    #         self.indices,
    #         measurements,
    #         shape=high_freq_measurements.shape
    #     ) 
    #     high_freq_measurements = high_freq_measurements


    #     low_freq_rec = self.high_scale_op.adj_op(low_freq_measurements)
    #     low_freq_grad = self.high_scale_op.adj_op(low_freq_measurements - y)

    #     high_freq_rec = self.high_scale_op.adj_op(high_freq_measurements)
    #     high_freq_grad = self.high_scale_op.adj_op(high_freq_measurements - y)
